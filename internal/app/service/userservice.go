...
func NewUserService(config func() *hocon.Config, database *sqlx.DB) *UserService {
...
func (s *UserService) LoginGoogleUser(ctx context.Context, googleUser *structs.GoogleUserInfo) (*structs.User, error) {
	logger := logdoc.GetLogger()
	...
	var u *structs.User
	u, err := s.r.FindUserByLogin(ctx, googleUser.Email)
	if err != nil {
		span.SetTag("error", true)
		return nil, err
	}
	...
	if u != nil {
		logger.Info(fmt.Sprintf("google вход. пользователь с почтой %s уже существует", u.Email))
		return u, nil
	}
...
func (s *UserService) LoginMailRuUser(ctx context.Context, mailRuUser *structs.MailRuUserInfo) (*structs.User, error) {
	logger := logdoc.GetLogger()
	...
	var u *structs.User
	u, err := s.r.FindUserByLogin(ctx, mailRuUser.Email)
	if err != nil {
		span.SetTag("error", true)
		return nil, err
	}
	...
	if u != nil {
		logger.Info(fmt.Sprintf("mailru вход. пользователь с почтой %s уже существует", u.Email))
		return u, nil
	}
	...
	u.Sub = strconv.Itoa(mailRuUser.ID)

	return u, nil
}

func (s *UserService) LoginVKUser(ctx context.Context, vkUser *structs.VKUser) (*structs.User, error) {
	logger := logdoc.GetLogger()
	...
	var u *structs.User
	u, err := s.r.FindUserByLogin(ctx, vkUser.Email)
	if err != nil {
		span.SetTag("error", true)
		return nil, err
	}
	...
	if u != nil {
		logger.Info(fmt.Sprintf("VK вход. пользователь с почтой %s уже существует", u.Email))
		return u, nil
	}
...
func (s *UserService) LoginYandexUser(ctx context.Context, userInfo *structs.YandexUserInfo) (*structs.User, error) {
	logger := logdoc.GetLogger()

	span, ctx := opentracing.StartSpanFromContext(ctx, "login yandex user service")
	defer span.Finish()

	var u *structs.User
	u, err := s.r.FindUserByLogin(ctx, userInfo.DefaultEmail)
	if err != nil {
		span.SetTag("error", true)
		return nil, err
	}
	...
	if u != nil {
		logger.Info(fmt.Sprintf("Yandex вход. пользователь с почтой %s уже существует", u.Email))
		return u, nil
	}

	u, err = s.r.FindUserByLoginAndSystem(ctx, userInfo.DefaultEmail, "vk")
	if err != nil {
		span.SetTag("error", true)
		return nil, err
	}
	...
	u, err = s.r.CreateYandexUser(ctx, userInfo)
	if err != nil {
		span.SetTag("error", true)
		return nil, err
	}
